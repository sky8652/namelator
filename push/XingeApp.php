...
class XingeApp
{
    ...
    public static function PushAccountAndroid($accessId, $secretKey, $title, $content, $account)
    {
        ...
        $action = new ClickAction();
        $action->setActionType(ClickAction::TYPE_ACTIVITY);
        $action->setActivity("com.lejun.zx.messages.mainMessage");
        $mess->setAction($action);
        $custom = array('itype'=>'gogoa', 'key2'=>'value2');
     ...
     * 使用默认设置推送消息给单个android版账户
     */
    public static function PushAccountAndroidWithParam($accessId, $secretKey, $title, $content, $account, $param)
    {
        $push = new XingeApp($accessId, $secretKey);
        $mess = new Message();
     ...
     * 使用默认设置推送消息给单个android版账户
     */
    public static function PushAccountAndroidWithParamType($accessId, $secretKey, $title, $content, $account, $param, $type)
    {
        $push = new XingeApp($accessId, $secretKey);
        $mess = new Message();
        $mess->setTitle($title);
        ...
        $action->setActivity("com.lejun.zx.messages.mainMessage");
        $mess->setAction($action);
        $custom = array('param'=>$param, 'type'=>$type);
		$mess->setCustom($custom);
    ...
    public static function PushTagAndroidWithParamType($accessId, $secretKey, $title, $content, $tag, $param, $type)
    {
        $push = new XingeApp($accessId, $secretKey);
        $mess = new Message();
        $mess->setTitle($title);
    ...
    public static function PushTagIosWithParamType($accessId, $secretKey, $content, $tag, $environment, $param, $type)
    {
        $push = new XingeApp($accessId, $secretKey);
        $mess = new MessageIOS();
        $mess->setAlert($content);
